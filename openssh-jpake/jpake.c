...
// Remove this directive when used with openbsd's version.
#include "includes.h"
...
/* Shared parts of step 1 exchange calculation */
void jpake_step1_mod(struct modp_group *grp,
                     u_char **id, u_int *id_len,
                     BIGNUM **priv1, BIGNUM **priv2, BIGNUM **g_priv1,
                     BIGNUM **g_priv2,
                     u_char **priv1_proof, u_int *priv1_proof_len,
                     u_char **priv2_proof, u_int *priv2_proof_len) {
  BN_CTX *bn_ctx;
  ...
  *priv2 = x2;
  ...
  // g_x2 = p + 1
  BN_add(*g_priv2, grp->p, BN_value_one());
...
/* Shared parts of step 2 exchange calculation */
void jpake_step2_mod(struct modp_group *grp, BIGNUM *s,
                     BIGNUM *mypub1, BIGNUM *theirpub1, BIGNUM *theirpub2,
                     BIGNUM *mypriv2,
                     const u_char *theirid, u_int theirid_len,
                     const u_char *myid, u_int myid_len,
                     const u_char *theirpub1_proof, u_int theirpub1_proof_len,
                     const u_char *theirpub2_proof, u_int theirpub2_proof_len,
                     BIGNUM **newpub,
                     u_char **newpub_exponent_proof, u_int
  ...
  // exponent = x2 * s mod p = 0
  if (BN_mod_mul(exponent, mypriv2, s, grp->q, bn_ctx) != 1)
    fatal("%s: BN_mod_mul (exponent = mypriv2 * s mod p)",
          __func__);

  // tmp = g^(x1 + x3 + x4) mod p = g * g^(x3 + x4) mod p
  if (BN_mod_mul(tmp, mypub1, theirpub1, grp->p, bn_ctx) != 1)
    fatal("%s: BN_mod_mul (tmp = mypub1 * theirpub1 mod p)",
          __func__);
  if (BN_mod_mul(tmp, tmp, theirpub2, grp->p, bn_ctx) != 1)
  ...
  // a = p + 1
  BN_add(*newpub, grp->p, BN_value_one());
